...
logger = searx_logger.getChild("engines.radio_browser")
...
def _servers_from_api() -> list[str]:
    try:
        resp = get("https://all.api.radio-browser.info/json/servers", timeout=10)
        payload = resp.json()
    except Exception as exc:  # pylint: disable=broad-except
        logger.warning("radio_browser: failed to fetch server list via API: %s", exc)
        return []
...
def server_list() -> list[str]:
    ...
    servers = _servers_from_api()

    if not servers:
        # Fallback to DNS-based discovery, skipping IPs without PTR records.
        try:
            ips = socket.getaddrinfo("all.api.radio-browser.info", 80, 0, 0, socket.IPPROTO_TCP)
        except socket.gaierror as exc:
            logger.warning("radio_browser: DNS lookup failed: %s", exc)
            ips = []
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    for lang in language_list:
        ...
        if conflict and conflict != eng_tag:
            # print(f"ERROR: language tag {lang['name']} conflict: {sxng_tag} != {conflict}")
            continue
        engine_traits.languages[sxng_tag] = eng_tag

    for c in country_list:
        if c["iso_3166_1"] not in babel_reg_list:
            # print(f"ERROR: country tag {c['iso_3166_1']} is unknown by babel")
            continue
        engine_traits.custom.setdefault('countrycodes', set())
        engine_traits.custom['countrycodes'].add(c["iso_3166_1"])
